		...
		foreach($this->query($sql) as $row){
			...
			if ( $usePHPSNMP ) {
                            if($row["DiscretePowerForPorts"] != "" && $row["PowerOID"] != "") {
                                ...
                                list($foreOID, $backOID) = sscanf($index, "%s?%s");
                                // Build OID for every single Port
                                for ($i=0;$i < $row["NumOutlets"];$i++) {
                                    $portPower = sprintf("%s%s%s", $foreOID, "." . $i);
                                    if ($backOID != "") {
                                        $portPower = $portPower . $backOID;
                                    }
                                ...
                                for ($i=1;$i<4;$i++) {
                                    if ( $row["OID".$i] != "" ) {
                                        switch($row["SNMPVersion"]) {
                                            case "1":
                                                // TODO: Accidently interchanged the commands. Gotta fix that.
                                                $foo = explode( " ", @snmpget( $row["IPAddress"], $Community, $row["OID".$i] ));
                                                break;
                                            case "2c":
                                                $foo = explode( " ", @snmp2_get( $row["IPAddress"], $Community, $row["OID".$i] ));
                                                break;
                                        }
                                        switch ($i) {
                                            case 1: 
